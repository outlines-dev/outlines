...
TotalCompletionsType = Union[list[str], str]
...
class SequenceGeneratorAdapter:
    ...
    def strip_completions(
        self,
        completions: Union[str, List[str]],
        prompts: Union[str, List[str]],
        ...
        if isinstance(completions, str):
            ...
            return [
                completion[len(prompt) - len(aligned_prompt) :]
    ...
    def stream(
        ...
        **model_specific_params,
        ...
        def strip_text_chunks(
            stripped_completions: Union[str, List[str]],
            text_chunks: Union[str, List[str]],
        ) -> Union[str, List[str]]:
            """Get the stripped text_chunks from the stripped_completions."""
            if isinstance(stripped_completions, list):
                return [
                    stripped_completion[-len(text_chunk) :]
                    if len(text_chunk) > 0
                    else ""
                    for stripped_completion, text_chunk in zip(
                        stripped_completions, text_chunks
                    )
                ]
            else:
                return (
        ...
        total_completions: TotalCompletionsType
        ...
        for text_chunks in self.model.stream(
            ...
            **model_specific_params,
            ...
            if isinstance(total_completions, list):
                total_completions = [
                    total_completion + text_chunk
                    for total_completion, text_chunk in zip(
                        total_completions, text_chunks
                    )
                ]
            else:
                total_completions += text_chunks
            ...
            yield strip_text_chunks(stripped_completions, text_chunks)
