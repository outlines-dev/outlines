...
class FSMLogitsProcessor(OutlinesLogitsProcessor):
    ...
    def process_logits(
        ...
        if self._is_first_token:
            ...
            for i, seq_ids in enumerate(input_ids):
                ...
                curr_state = self._seq_fsms[i].get_next_state(prev_state, seq_ids[-1])
        ...
        for i, fsm_state in enumerate(sequence_states):
            allowed_tokens = self._seq_fsms[i].get_next_instruction(fsm_state).tokens
    ...
    def align_prompts(self, prompts: Union[str, List[str]]) -> Union[str, List[str]]:
        ...
        aligned_prompts = [
            fsm.align_prompt_tokens(prompt)
